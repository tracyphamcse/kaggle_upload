...
class BertBiLSTM_BAS(BertPreTrainedModel):
    
    def __init__(self, config):
        super(BertBiLSTM_BAS, self).__init__(config)
...
class BertBiLSTM_BAS_2(BertPreTrainedModel):
    
    def __init__(self, config):
        super(BertBiLSTM_BAS_2, self).__init__(config)
        ...
        self.lstm_1 = nn.LSTM(input_size=config.hidden_size, hidden_size=self.hidden_dim, num_layers=2, bidirectional=True)
        self.lstm_2 = nn.LSTM(input_size=config.hidden_size, hidden_size=self.hidden_dim, num_layers=2, bidirectional=True)

        self.hidden2dense = nn.Linear(config.hidden_size + self.hidden_dim * 2 * 2, self.relu_dim_1)
        self.dense2label = nn.Linear(self.relu_dim_1, self.config.num_labels)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(0.2)
    ...
    def init_hidden(self):
        # first is the hidden h
        # second is the cell c
        return (Variable(torch.zeros(4, self.batch_size, self.hidden_dim)).to(DEVICE),
                Variable(torch.zeros(4, self.batch_size, self.hidden_dim)).to(DEVICE))
...
class BertBow_BAS(BertPreTrainedModel):
    
    def __init__(self, config):
        super(BertBow_BAS, self).__init__(config)
...
class BertCNN_BAS(BertPreTrainedModel):
    
    def __init__(self, config):
        super(BertCNN_BAS, self).__init__(config)
